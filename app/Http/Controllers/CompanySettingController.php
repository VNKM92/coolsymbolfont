...
class CompanySettingController extends Controller
{
                    ...
                    if(!empty($dat_details->username_dat)){   
                	   ...
                	   \Log::info($user);
                	    ...
                	    //dd($res);
                		    if(!empty($res)){
                		         // dd($res);
                		        if(isset($res->statusCode) && $res->statusCode == 403){
                		            $mid = base64_decode($mid);
                		            return redirect('admin/setting/dat/user/api')->with('error', 'DAT username may be incorrect');
                		        }
                		        else{
                		          //  $mid = base64_decode($mid);setting/dat/user/api
                		          
                		           if(isset($res->statusCode) && $res->statusCode == 401){
                		               return redirect('admin/setting/dat/user/api')->with('error', 'Super Admin DAT Token Expire. Pls. Regenerate');
                		           }else{
                		              //   dd($res);
                		              if(isset($res->statusCode) && $res->statusCode == 500){
                		                 return redirect('admin/setting/dat/user/api')->with('error', 'Internal Server Error');
                		              }else{
    ...
    public function showadminapi(Request $request){
        ...
        //  $Userdata = User::where('dat_user_auth_status',$statusid)->get();
         ...
         $Userdata = DAT::whereNotIn('user_id',['1'])->get();
         
        return view('backend/settingManagement/apiDatinfoUser/adminapi',compact('data','Userdata'));
    }
    

    // 16-feb  = 2236 pay 7march
    // 17-feb to 15-march total exp
    ...
    public function rdatdelete(Request $request, $id){

        $id = base64_decode($id);
        $status = '0';
        // $updatedate = User::where('id',$id)->update(['dat_user_auth_status'=>$status]);

        
            $de_uid = DAT::where('id',$id)->pluck('user_id')->first();
            $updatedate = User::where('id',$de_uid)->update(['dat_user_auth_status'=>$status]);

            $deleteActivity = DAT::where('id',$id)->delete();
            if($deleteActivity){
                $status = '0';
                $updatedate = User::where('id',$de_uid)->update(['dat_user_auth_status'=>$status]);
                return back()->with('success', 'DAT User-Name Deleted Successfully..');
            }
            // if(!empty($id))
            // {
            // }    lucas.white@amblogistic.us
        // Session::flash('error', 'DAT username not created yet..!'); 
        return back()->with('error', 'DAT username not created yet..');

    }

    public function datdelete(Request $request, $id){
        ...
        $updatedate = User::where('id',$id)->update(['dat_user_auth_status'=>$status]);
            ...
            $deleteActivity = DAT::where('user_id',$id)->delete();
            if($deleteActivity){
                $status = '0';
                $updatedate = User::where('id',$id)->update(['dat_user_auth_status'=>$status]);
                return back()->with('success', 'DAT User-Name Deleted Successfully..');
