...
class CompanyController extends Controller
{
    ...
    public function disputedlist(Request $request){
        if (is_null($this->user) || !$this->user->can('shipper-all')) {
            abort(403, 'Sorry !! You are Unauthorized to view any admin !');
        }
        $GetUser =  User::where('user_type', 'agent')->orwhere('user_type', 'officer')->get();
        ...
        //echo '<pre>'; print_r($comp_data);die; 
        $page = 'shipper_details';
        return view('backend.shipmentManagement.shipper.disputed.index', compact('page','GetUser','comp_data','total','shipper_pending','shipper_approve','shipper_disapprove'));
    }

    

    public function disputedshipper(Request $request){
        if (is_null($this->user) || !$this->user->can('shipper-all')) {
            abort(403, 'Sorry !! You are Unauthorized to view any admin !');
        }
        $GetUser =  User::where('user_type', 'agent')->orwhere('user_type', 'officer')->get();
        ...
        //echo '<pre>'; print_r($comp_data);die; 
        $page = 'shipper_details';
        return view('backend.shipmentManagement.shipper.disputed.list', compact('page','GetUser','comp_data','total','shipper_pending','shipper_approve','shipper_disapprove'));
    }

    
    public function shipperdisputed(Request $request){
        $response = [];
            if($request->isMethod('post')){
               ...
               // $statusid = $data['cat'];
                ...
                $shipmentget = Company::where('id',$statusid)->first();
                // $shipmentget->ap_access_date = date('Y-m-d');
                $shipmentget->shipper_disputed = $status;
                $shipmentget->save();
    
                if($shipmentget){
                    $response['status'] = 'true';
                    $response['msg']    = 'Shipper Blocked Status updated successfully';
                } else{
    ...
    public function show(Request $request,$shipper_id){
        ...
        // $shipper_id = base64_encode($shipper_id);
    ...
    public function edit(Request $request,$shipper_id){
        ...
        if($shipper_details){
            ...
            if ($request->isMethod('post')){
                ...
                $shipper_details->bill_zipcode	= $data['bill_zipcode'];
                //payment details
                // $shipper_details->pre_pay	        = $data['shipper_prepaid'];
                // $shipper_details->shipper_prepaid	= $data['shipper_prepaid'];
                $shipper_details->payment_terms	    = $data['payment_terms'];
                $shipper_details->payment_method	= $data['payment_method'];
                $shipper_details->invoice_method	= $data['invoice_method'];
    ...
    public function request_update_Old(Request $request){
        $data = $request->all();
        //dd($data);
        $user_id = Auth::id();
        $user = User::where('id',$user_id)->first();
        if($request->isMethod('post')){
            ...
            //     'assigntoid' => 'required',
            // ]);
            // if(empty($data['assigntoid'])){
            //     return Redirect::back()->with('assigntoid');
            // }
            // dd($data);
            ...
            if($data['shipper_prepaid'] == 0){
                // $prepay = '0';
                $prepay  = $data['shipper_prepaid'];
                $prelimit = '0';
                // $prelimit = 'NULL';
            }else{
                // $prepay = '1';
            ...
            // $prepay  = $data['shipper_prepaid'];
            if($data['status'] == '1'){
                $upd = Company::where('id',$data['companies_id'] )->update(['un_secured_limit'=>$prelimit,'pre_pay'=>$prepay,'credit_limit'=>$data['limit'],'approved'=>$data['status'],'assigntoid'=>$data['assigntoid'], 'shipper_prepaid'=>$prepay
                ...
                $req_comment->assigntoname =  $data['assigntoid'];
                ...
                $req_comment->save();
                    ...
                    if($req_comment->save()){
                        ...
                        $req_history->assigntoname =  $data['assigntoid'];
                ...
                $req_comment->receiver_id = $agent['id'];
                $req_comment->companies_id = $data['companies_id'];
                $req_comment->comment = ltrim($data['comment']);
                $req_comment->request_date = date('Y-m-d');
    ...
    public function request_update(Request $request){
        ...
        if($request->isMethod('post')){
            ...
            if($data['status'] == '1'){
                $upd = Company::where('id',$data['companies_id'] )->update(['un_secured_limit'=>$prelimit,'pre_pay'=>$prepay,'credit_limit'=>$limit,'approved'=>$data['status'],'assigntoid'=>$data['assigntoid'], 'shipper_prepaid'=>$prepay, 'payment_terms'=>$data['payment_terms']
            ]); 
                ...
                $req_comment->save();
                    ...
                    if($req_comment->save()){
                        ...
                        $req_history->credit_limit      = $data['limit'];
                        $req_history->approved          = $data['status'];
                        $req_history->payment_terms     = $data['payment_terms'];
                        $req_history->comment          = $data['comment'];
                        $req_history->assigntoname =  $data['assigntoid'];
                        // $req_history->comment          = ltrim($str($data['comment']));
                        $req_history->user_id           = $uid;
    ...
    public function srequsetLog(){
        ...
        $shiper_log = Shipper_request::get();
			...
			if($user_id == 1){
				...
				$ship = Shipper_request::where('sender_id',$user_id)->orderBy('id','DESC')->get();
			}
