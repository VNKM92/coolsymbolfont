@extends('frontend.layouts.master')
@section('title', $data->metatitle)
@section('description', $data->metadiscription)
@section('content')


<main class="flex-grow pt-6">

    <div class="bg-white  sm:py-10">
        <div class="mx-auto max-w-7xl px-6 lg:px-8">
          <div class="mx-auto lg:mx-0 ">
            @php 
                    $imag ="";
                    if(!empty($data->image)){
                        $imag = url('public/backend/movie/blog').'/'.$data->image ;
                    }else {
                        $imag = "https://coolsymbol.online/public/frontend/assets/blog/blognew-post.png" ;
                    }
            @endphp
            
        
            <figure class="grid h-screen place-items-center">
                
              <img class="object-center max-w-full rounded-lg  img-responsive  max-width:100%  size-120 width:100%" src="{{ $imag }}" alt="{!! $data->title !!}">
           
              <h2 class="text-2xl font-bold text-slate-900 dark:text-white mt-3 mb-3 group-hover:text-sky-600 transition-colors">
                    {!! $data->title !!}
                </h2>
            </figure>
    
            
            <p class="mt-2 text-lg/8 text-gray-600"> {!! substr($data->description ,0, 8000, ) ?? "Description Not Available" !!}</p>
          </div>
          <div
            class="mx-auto mt-10 grid max-w-2xl grid-cols-1 gap-x-8 gap-y-16 border-t border-gray-200 pt-10 sm:mt-16 sm:pt-16 lg:mx-0 lg:max-w-none lg:grid-cols-3">
            <!--<article class="flex max-w-xl flex-col items-start justify-between">-->
            <!--  <div class="flex items-center gap-x-4 text-xs">-->
            <!--    <time datetime="2020-03-16" class="text-gray-500">Mar 16, 2020</time>-->
            <!--    <a href="#"-->
            <!--      class="relative z-10 rounded-full bg-gray-50 px-3 py-1.5 font-medium text-gray-600 hover:bg-gray-100">Marketing</a>-->
            <!--  </div>-->
            <!--  <div class="group relative">-->
            <!--    <h3 class="mt-3 text-lg/6 font-semibold text-gray-900 group-hover:text-gray-600">-->
            <!--      <a href="#">-->
            <!--        <span class="absolute inset-0"></span>-->
            <!--        Boost your conversion rate-->
            <!--      </a>-->
            <!--    </h3>-->
            <!--    <p class="mt-5 line-clamp-3 text-sm/6 text-gray-600">Illo sint voluptas. Error voluptates culpa-->
            <!--      eligendi. Hic vel totam vitae illo. Non aliquid explicabo necessitatibus unde. Sed-->
            <!--      exercitationem placeat consectetur nulla deserunt vel. Iusto corrupti dicta.</p>-->
            <!--  </div>-->
            <!--  <div class="relative mt-8 flex items-center gap-x-4">-->
            <!--    <img-->
            <!--      src="https://images.unsplash.com/photo-1519244703995-f4e0f30006d5?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=facearea&facepad=2&w=256&h=256&q=80"-->
            <!--      alt="" class="size-10 rounded-full bg-gray-50" />-->
            <!--    <div class="text-sm/6">-->
            <!--      <p class="font-semibold text-gray-900">-->
            <!--        <a href="#">-->
            <!--          <span class="absolute inset-0"></span>-->
            <!--          Michael Foster-->
            <!--        </a>-->
            <!--      </p>-->
            <!--      <p class="text-gray-600">Co-Founder / CTO</p>-->
            <!--    </div>-->
            <!--  </div>-->
            <!--</article>-->
    
              
          </div>
        </div>
    </div>

</main>

@endsection