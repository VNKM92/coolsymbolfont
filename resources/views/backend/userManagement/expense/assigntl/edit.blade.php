<?php 
    if(isset($details)){
        $title = 'Edit';
        $form_id = 'ExpenseForm';
        $action  =  url('/admin/user/expense/assign/edit'.$id);
    }else{
        $title = 'Add';
        $form_id = 'ExpenseForm';
...
<div class="page-wrapper">
    <div class="page-content">
        ...
        <ul class="nav nav-tabs">
                    ...
                    <li class="pending_approval active"><a href="{{  $action }}" data-toggle="tab" aria-expanded="true">{{$title}} Assign</a>
                    </li>
                </ul>
                                        ...
                                        @endphp
                                            ...
                                            <select name="assign_tl" id="assign_tl" class="form-control" required>
                                                ...
                                                @foreach ($tlusers as $user)
                                                        <option value="{{ $user->id }}" {{  $user->id == $expdata->assign_tl ? 'selected' : '' }}>{{ $user->name }}
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select name="assign_to_agent" id="assign_to_agent" class="form-control" required>
                                                ...
                                                @foreach ($agentusers as $user)
                                                        <option value="{{ $user->id }}" {{  $user->id == $expdata->assign_to_agent ? 'selected' : '' }}>{{ $user->name }}</option>
                                                @endforeach
